...
function wpgrade_prepare_password_for_custom_post_types() {
	...
	$wpgrade_private_post = pile::is_password_protected();
		...
		function add_video_url_field_to_attachments( $form_fields, $post ) {
			...
			if ( ! isset( $form_fields["link_media_to"] ) ) {
				...
				$select_options = array(
					'none'             => __( 'None', wpgrade::textdomain() ),
					'media_file'       => __( 'Media File', wpgrade::textdomain() ),
					'custom_image_url' => __( 'Custom Image URL', wpgrade::textdomain() ),
					'custom_video_url' => __( 'Custom Video URL', wpgrade::textdomain() ),
					'external'         => __( 'External URL', wpgrade::textdomain() )
				);
				...
				$form_fields["link_media_to"] = array(
					'label' => __( 'Linked To', wpgrade::textdomain() ),
					'input' => 'html',
					'html'  => $select_html
				);
			}
			...
			if ( ! isset( $form_fields["video_url"] ) && !empty( $link_media_to_value ) && $link_media_to_value == 'custom_video_url' ) {
				$form_fields["video_url"] = array(
					"label" => __( "Custom Video URL", wpgrade::textdomain() ),
			...
			if ( ! isset( $form_fields["custom_image_url"] ) && !empty( $link_media_to_value ) && $link_media_to_value == 'custom_image_url' ) {
				$form_fields["custom_image_url"] = array(
					"label" => __( "Custom Image URL", wpgrade::textdomain() ),
					"input" => "text", // this is default if "input" is omitted
					"value" => esc_url( get_post_meta( $post->ID, "_custom_image_url", true ) ),
					"helps" => __( "<p class='desc'>Link this image to a custom url.</p>", wpgrade::textdomain() ),
				);
			}
...
function pile_body_classes( $classes ) {
	if ( is_single() && 'post' == get_post_type() && is_active_sidebar( 'sidebar-main' ) &&  wpgrade::option( 'blog_single_show_sidebar' )) {
		$classes[ ] = 'has_sidebar';
	}
